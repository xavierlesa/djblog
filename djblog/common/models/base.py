...
from django.template.defaultfilters import (
        slugify, striptags, linebreaksbr, urlize, truncatewords,
        truncatewords_html, force_escape, escape, urlizetrunc
        )
    ...
    if django.get_version() < '1.9':
        settings.TEMPLATE_CONTEXT_PROCESSORS = settings.TEMPLATE_CONTEXT_PROCESSORS + \
                                               (
                                                       ...
                                                       'djblog.context_processors.site'
                                                       )
    else:
        settings.TEMPLATE_CONTEXT_PROCESSORS = settings.TEMPLATE_CONTEXT_PROCESSORS + \
                                               [
...
if getattr(settings, 'TEMPLATES', False) and len(settings.TEMPLATES): # Django 1.8
    if 'django.template.context_processors.i18n' not in settings.TEMPLATES[0]['OPTIONS']['context_processors']:
        settings.TEMPLATES[0]['OPTIONS']['context_processors'] = \
                settings.TEMPLATES[0]['OPTIONS']['context_processors'] + ['django.template.context_processors.i18n',]
...
if getattr(settings, 'TEMPLATES', False) and len(settings.TEMPLATES): # Django 1.8
    if 'djblog.context_processors.site' not in settings.TEMPLATES[0]['OPTIONS']['context_processors']:
        settings.TEMPLATES[0]['OPTIONS']['context_processors'] = \
                settings.TEMPLATES[0]['OPTIONS']['context_processors'] + ['djblog.context_processors.site',]
...
#            m = "Existe una instancia %s con estos (key) (val) para estos site(s) %s" % \
#                                                   (model_instance.__class__, filter_keys)
...
class BaseModel(MultiSiteBaseModel):
    """
    BaseModel esta pensado para objectos que requieren (aunque opcionales)
    del uso de los meta_keywords y meta_description. También ofrece una base 
    para uso de multi-lenguajes, multi-sitios, auto slug, fecha de publicación
    y los campos de -invisibilidad- y activo/inactivo, éste definitivamente es
    un buen cominezo para los futuros modelos.
    """
...
class ContentModel(models.Model):
    ...
    copete = models.TextField(verbose_name=_(u"Copete"),
                              help_text=_(u"Es opcional, y si exsite se usa para el extracto"), blank=True)
    content = models.TextField(verbose_name=_(u"Contenido en texto plano"))
    content_rendered = models.TextField(verbose_name=_(u"Contenido HTML"), 
                                        help_text=_(u"Este es el contenido a mostrarse, permite marcado"))
...
class GenericRelationModel(models.Model):
    content_type = models.ForeignKey(ContentType, blank=True, null=True,
                                     verbose_name=_('content type'), related_name="content_type_set_for_%(class)s")
